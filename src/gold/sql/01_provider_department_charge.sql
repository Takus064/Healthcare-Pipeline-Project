...
FROM `fpt-fresher-495407.silver_dataset.transactions` AS t -- Join bảng departments
    LEFT JOIN `fpt-fresher-495407.silver_dataset.departments` AS d ON t.dept_id = d.dept_id
    AND t._source_system = d._source_system -- SCD Type 2 Join cho bảng providers (lấy bản ghi hiện tại)
    AND d._is_quarantined = FALSE
